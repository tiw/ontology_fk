...
class ObjectSetService:
    ...
    def get_base_object_set(
        self, object_type: ObjectType, principal_id: str = None
    ) -> ObjectSet:
        ...
        if object_type.permissions and principal_id:
            if not object_type.permissions.check(principal_id, PermissionType.VIEW):
                raise PermissionError(
                    f"User {principal_id} does not have VIEW permission for {object_type.api_name}"
                )
...
class ActionService:
    ...
    def execute_action(
        ...
        if action_type.permissions:
            if not action_type.permissions.check(
                principal.id, PermissionType.EDIT
            ):  # Assuming EDIT permission needed for Action
                raise PermissionError(
                    f"User {principal.id} does not have permission to execute action {action_type_api_name}"
                )
