import unittest
from ontology_framework import Ontology, ObjectType, LinkType, ObjectInstance, ObjectSet, PropertyType
...
class TestLinks(unittest.TestCase):
    def setUp(self):
        ...
        # Define Types
        factory = ObjectType(api_name="Factory", display_name="Factory", primary_key="id")
        factory.add_property("id", PropertyType.STRING)
        
        equipment = ObjectType(api_name="Equipment", display_name="Equipment", primary_key="id")
    ...
    def test_search_around(self):
        ...
        if f1_obj.primary_key_value != "f1":
            f1_obj = [o for o in self.ontology.get_objects_of_type("Factory") if o.primary_key_value == "f1"][0]
            
        start_set = ObjectSet(self.ontology.get_object_type("Factory"), [f1_obj], self.ontology)
    ...
    def test_permissions(self):
        ...
        self.ontology.create_link("FactoryHasEquipment", "f1", "e1", user_permissions=["EDIT_LINK_FactoryHasEquipment"])
        
        # Test with missing permission
        with self.assertRaisesRegex(PermissionError, "Missing permission: EDIT_LINK_FactoryHasEquipment"):
            self.ontology.create_link("FactoryHasEquipment", "f1", "e2", user_permissions=["SOME_OTHER_PERM"])
            
        # Test delete with missing permission
        with self.assertRaisesRegex(PermissionError, "Missing permission: EDIT_LINK_FactoryHasEquipment"):
            self.ontology.delete_link("FactoryHasEquipment", "f1", "e1", user_permissions=["SOME_OTHER_PERM"])

        # Test delete with valid permission
        self.ontology.delete_link("FactoryHasEquipment", "f1", "e1", user_permissions=["EDIT_LINK_FactoryHasEquipment"])
